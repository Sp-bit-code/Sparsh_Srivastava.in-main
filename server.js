...
function generateSpeech(text) {
  return new Promise((resolve, reject) => {
    ...
    const pythonCommand = process.env.PYTHON_COMMAND || "python3";
    const assistantVoice = process.env.AssistantVoice || "en-IN-NeerjaNeural";

    console.log("----- TTS START -----");
    console.log("Python command:", pythonCommand);
    console.log("TTS script path:", TTS_SCRIPT_PATH);
    console.log("Speech output path:", SPEECH_PATH);
    console.log("AssistantVoice:", assistantVoice);
    ...
    child.on("close", (code) => {
      ...
      console.log("TTS stdout:", output || "No stdout");
      console.log("TTS stderr:", errorOutput || "No stderr");
      console.log("Speech exists:", fs.existsSync(SPEECH_PATH));
      console.log("----- TTS END -----");
      ...
      if (code === 0 && fs.existsSync(SPEECH_PATH)) {
        ...
        reject(
          new Error(
            errorOutput ||
              output ||
              `TextToSpeech.py exited with code ${code}`
          )
        );
      }
    });
...
app.post("/api/text-to-speech", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      audioUrl: `/api/speech?t=${Date.now()}`,
    });
  } catch (error) {
