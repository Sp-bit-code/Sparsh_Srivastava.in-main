    ...
    try {
      ...
      if (!ttsResponse.ok) {
        ...
        try {
          ...
          try {
            errorMessage = await ttsResponse.text();
          } catch {
            errorMessage = "Unable to read TTS error response";
          }
        }

        console.error("TTS backend error:", errorMessage);
        return;
      }

      const data = await ttsResponse.json();
      const audioUrl = data.audioUrl || `/api/speech?t=${Date.now()}`;
